...
## Problem 1 - Iron Bank
```{r}
sim_trade_flips = do(100000) * nflip(n = 2021, prob = 0.024) # Simulating the probability test 100000 times
...
```
The null hypothesis of this problem is that the judge's jury selection follows the same racial distribution as the county's eligible pool. The test statistic I used to measure the evidence against the null hypothesis is the chi_square_statistic function, which measures how different the observed juror numbers were from the expected juror numbers. I simulated 10,000 jury selections based on the expected proportion. The p-value, 0.0152, is less than 0.05, which suggests that the judge's selection deviates from the county's demographics.
...
The sentence with the lowest p-value of 0.000 is most likely the one that has been watermarked by the LLM. The sentence is number 6, and it has a lot of characters that would be infrequent in a "human-generated" sentence.